...
median_admissions %>%
  filter(year == 2019) %>%
  rename(alcohol_admissions = median_admissions) %>%
  ggplot(aes(x=reorder(health_board,alcohol_admissions), y=alcohol_admissions, colour = health_board)) + 
  geom_bar(stat = "identity")) +
  coord_flip() + 
  xlab(NULL) + 
  ylab("Age-sex standardised rate per 100,000") + 
...
median_admissions %>%
  filter(year == 2019) %>%
  rename(alcohol_admissions = median_admissions) %>%
  ggplot(aes(x=health_board, y=alcohol_admissions, fill=health_board !='Grampian')) + 
  geom_bar(stat = "identity") +
  coord_flip() +                         
  xlab(NULL) + 
  ylab("Age-sex standardised rate per 100,000") + 
...
#Need to install the package 'viridis'
install.packages(viridis)
library(viridis)

#Now redraw the graph using the viridis colour schemes
median_admissions %>%
  filter(year %in% c(2010,2015,2019)) %>%
  mutate(year = as.factor(year)) %>%        
  rename(alcohol_admissions = median_admissions) %>%
  ...
  ylab("Age-sex standardised rate per 100,000") + 
  ggtitle("Alcohol-related hospital admissions") + 
  scale_fill_viridis_d(alpha = 0.75, begin = 0.9, end = 0) +
  theme_bw()
```
...
Add your code here!
```{r}
median_admissions <- left_join(median_admissions, deprived_ratio, by = c('health_board' = 'NHS health board'))
...
median_admissions %>% 
  filter(year == 2019) %>%
  ggplot(aes(x = percent_deprived, y = median_admissions)) +
  geom_point() +
  geom_text(aes(label=health_board), size = 2, nudge_y = 20, nudge_x = -0.015)
  xlab("deprivation decile") + 
  ylab("median alcohol-related admissions") + 
  ggtitle("Alcohol-related hospital admissions in 2019 by deprivation decile") + 
  theme_bw()
```
